export const products = [
  {
    id: 1,
    name: 'Aluminium Alloys Fan Set & Hub (Less Vibration)',
    bannerImage: 'assets/gallery/fan-hub-assembly.jpg',
    secondImage: 'assets/gallery/industrial-fan-blade.jpg',
    description:
      'Precision dynamically balanced cast aluminium alloy fan hub and aerofoil blade set engineered for maximum CFM air movement with significantly less vibration, protecting motor bearings and eliminating mechanical stress.',
    applications: [
      'Industrial cooling towers',
      'HVAC chiller plants',
      'Thermal & captive power plants',
      'Chemical & pharmaceutical facilities',
    ],
    advantages: [
      'Aerodynamic aluminium fan blades for less vibration & zero flutter',
      'Lightweight high-strength cast alloy construction',
      'Dynamic balance protects motor bearings & extended drive shafts',
      'Aerodynamically profiled for high CFM and low noise',
    ],
  },
  {
    id: 2,
    name: 'Aluminium Alloys Sprinkler Assembly',
    bannerImage: 'assets/gallery/aluminium-sprinkler.jpg',
    secondImage: 'assets/gallery/pvc-end-cap-63mm.jpg',
    description:
      'Advanced 360° rotary sprinkler head offering uniform, low-pressure water distribution across cooling tower fill media for peak evaporative heat transfer.',
    applications: [
      'Round bottle cooling towers',
      'Cooling tower fill irrigation',
      'Chemical processing plants',
      'Industrial oil refineries',
    ],
    advantages: [
      'Uniform 360-degree water spread',
      'Low operating pressure requirement',
      'Self-cleaning rotary orifice design',
      'High-grade anti-corrosive alloy',
    ],
  },
  {
    id: 3,
    name: 'PVC Fills',
    bannerImage: 'assets/products/pvc-fills.jpg',
    secondImage: 'assets/slide3.jpg',
    description:
      'High-performance honeycomb & corrugated PVC fill media designed to drastically increase air-water surface contact time for accelerated thermal dissipation.',
    applications: [
      'Crossflow & counterflow cooling towers',
      'Power station condensing systems',
      'Commercial building central HVAC',
    ],
    advantages: [
      'Engineered cross-flute honeycomb geometry',
      'UV and chemical resistant virgin PVC',
      'High structural load capacity',
      'Fire-retardant formulation',
    ],
  },
  {
    id: 4,
    name: 'PVC Splash Cap',
    bannerImage: 'assets/products/pvc-splash-cap.jpg',
    secondImage: 'assets/products/distribution-branch-pipe.jpg',
    description:
      'Targeted splash caps that ensure optimal kinetic water breakup across upper deck nozzles for maximum evaporative cooling contact.',
    applications: ['Industrial cooling processes', 'Chemical/petroleum plants', 'Square & package cooling towers'],
    advantages: [
      'Robust molded PVC construction',
      'Quick snap-fit replacement',
      'Improves micro-droplet water breakup',
    ],
  },
  {
    id: 5,
    name: 'PVC End Cap (63 mm)',
    bannerImage: 'assets/gallery/pvc-end-cap-63mm.jpg',
    secondImage: 'assets/gallery/tie-rod-adjuster.jpg',
    description:
      'Precision 63mm virgin PVC end caps manufactured for tight, leak-free sealing of cooling tower distribution branches and manifold pipe ends.',
    applications: ['Cooling tower fill header assembly', 'Water distribution pipe manifolds', 'Industrial fluid channels'],
    advantages: ['Guaranteed zero-leak sealing fit', 'Chemical & chlorine corrosion proof', 'Quick push-and-lock installation'],
  },
  {
    id: 6,
    name: 'PVC Drift Eliminator',
    bannerImage: 'assets/products/pvc-drift-eliminator.jpg',
    secondImage: 'assets/products/pvc-fills.jpg',
    description:
      'Aerodynamically contoured cellular eliminators that capture escaping water droplets to reduce drift loss below 0.005% of circulating water.',
    applications: ['Commercial building rooftop towers', 'Industrial process towers', 'Urban environment installations'],
    advantages: [
      'Reduces water makeup consumption',
      'Minimal pressure drop on exhaust fan',
      'High rigidity and interlocking modules',
    ],
  },
  {
    id: 7,
    name: 'Industrial Tie Rod',
    bannerImage: 'assets/products/industrial-tie-rod.jpg',
    secondImage: 'assets/gallery/tie-rod-adjuster.jpg',
    description:
      'High-tensile steel tie rods with anti-rust coatings providing critical internal structural bracing for cooling tower casing panels and fill supports.',
    applications: ['Tower structural casing assemblies', 'Large square and rectangular industrial towers'],
    advantages: [
      'High tensile load bearing strength',
      'Hot-dip galvanized & epoxy options',
      'Universal compatibility across tower models',
    ],
  },
  {
    id: 8,
    name: 'Tie Rod Adjuster',
    bannerImage: 'assets/gallery/tie-rod-adjuster.jpg',
    secondImage: 'assets/gallery/factory-plant.jpg',
    description:
      'Heavy-duty turnbuckle-style tie rod tension adjuster ensuring micro-adjustable, rock-solid diagonal and transverse tower bracing.',
    applications: ['Tower structural tension systems', 'Frame alignment and vibration suppression'],
    advantages: [
      'Smooth bidirectional tension adjustment',
      'High-grade forged metal construction',
      'Maintains structural geometry under heavy wind/water loads',
    ],
  },
  {
    id: 9,
    name: 'GI Clamp & Fastener Sets',
    bannerImage: 'assets/products/gi-pipe-clamp.jpg',
    secondImage: 'assets/products/industrial-tie-rod.jpg',
    description:
      'Galvanized Iron and stainless steel pipe clamps engineered for securing high-vibration headers, branch lines, and structural trusses.',
    applications: ['Pipe mounting and support', 'Framework reinforcement', 'Fill pack hanger brackets'],
    advantages: [
      'Rugged heavy-gauge galvanized finish',
      'Anti-corrosive against chlorinated water',
      'Easy bolt-on installation',
    ],
  },
  {
    id: 10,
    name: 'Distribution Branch Pipe',
    bannerImage: 'assets/products/distribution-branch-pipe.jpg',
    secondImage: 'assets/products/pvc-splash-cap.jpg',
    description:
      'Precision-drilled PVC / FRP branch pipes delivering balanced hydraulic flow from main headers to individual nozzle orifices.',
    applications: ['Cooling tower water distribution systems', 'Manifold retrofits and repairs'],
    advantages: ['Uniform hydraulic flow balancing', 'Anti-scaling smooth internal walls', 'Easy flange connection'],
  },
  {
    id: 11,
    name: 'FRP Flapper & Louvers',
    bannerImage: 'assets/products/frp-louvers-flapper.jpg',
    secondImage: 'assets/products/pvc-drift-eliminator.jpg',
    description:
      'Fiberglass Reinforced Plastic flappers and air-intake louvers that prevent water splash-out while directing optimal laminar airflow into the basin.',
    applications: ['Air inlet louvers', 'Basin splash containment', 'Industrial fluid channels'],
    advantages: [
      'Lightweight yet structurally rigid',
      '100% weather, rot, and rust proof',
      'Prevents debris ingestion into basin',
    ],
  },
  {
    id: 12,
    name: 'FRP / Aluminium Fan Blades',
    bannerImage: 'assets/gallery/industrial-fan-blade.jpg',
    secondImage: 'assets/gallery/fan-hub-assembly.jpg',
    description:
      'High-efficiency aerofoil fan blades engineered with adjustable pitch angles to maximize volumetric airflow (CFM) while reducing kWh electrical consumption.',
    applications: ['Induced draft cooling towers', 'Forced draft cooling towers', 'Heavy industrial HVAC'],
    advantages: [
      'High airflow efficiency with low power draw',
      'Severe chemical & moisture resistance',
      'Adjustable blade pitch for seasonal tuning',
    ],
  },
  {
    id: 13,
    name: 'Long Shaft Cooling Tower Motor',
    bannerImage: 'assets/products/long-shaft-motor.jpg',
    secondImage: 'assets/products/frp-fan-canopy.jpg',
    description:
      'IP55/IP66 weather-protected flange-mounted motors featuring extended stainless steel shafts designed for direct-drive fan operation in saturated air streams.',
    applications: ['Direct-drive cooling tower fan systems', 'Continuous industrial operations'],
    advantages: [
      'Cast-iron hermetic moisture protection',
      'Extended heavy-duty drive shaft',
      'IE3 high-efficiency energy rating',
    ],
  },
  {
    id: 14,
    name: 'FRP Fan Canopy & Venturi Stack',
    bannerImage: 'assets/products/frp-fan-canopy.jpg',
    secondImage: 'assets/products/long-shaft-motor.jpg',
    description:
      'Aerodynamically designed FRP venturi fan recovery cylinders that convert fan exit velocity into pressure, reducing horsepower requirements by up to 15%.',
    applications: ['Top discharge cooling towers', 'Noise-sensitive industrial & commercial sites'],
    advantages: [
      'Venturi effect reduces motor energy consumption',
      'UV-stabilized resin prevents weather degradation',
      'Significantly lowers discharge turbulence and acoustic noise',
    ],
  },
  {
    id: 15,
    name: 'Chiller & DG Cooling Tower Systems',
    bannerImage: 'assets/gallery/chiller-dg-installation-1.jpg',
    secondImage: 'assets/gallery/chiller-dg-rooftop-2.jpg',
    description:
      'Complete turnkey industrial cooling tower installations designed and manufactured for heavy commercial chiller plants and industrial diesel generator (DG) continuous duty.',
    applications: ['Industrial manufacturing plants', 'Hospitals and IT parks', 'Commercial real estate complexes'],
    advantages: [
      'End-to-end manufacturing and commissioning',
      'Punctual delivery and turnkey piping support',
      'Direct-from-manufacturer warranty and spares',
    ],
  },
];
